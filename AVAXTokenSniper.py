...
if checkSourceCode == "True" and (checkMintFunction == "True" or checkHoneypot == "True" or checkTraderJoeRouter == "True"):
    enableMiniAudit = True

def updateTitle():
    walletBalance = web3.fromWei(web3.eth.get_balance(walletAddress),'ether') #There are references to ether in the code but it's set to AVAX, its just how Web3 was originally designed
    ...
    # ctypes.windll.kernel32.SetConsoleTitleW("AVAXTokenSniper | Tokens Detected: " + str(numTokensDetected) + " | Tokens Bought: " + str(numTokensBought) + " | Wallet Balance: " + str(walletBalance) + " AVAX")
...
def Buy(tokenAddress, tokenSymbol):
    if(tokenAddress != None):
        ...
        0, # Set to 0 or specify min number of tokens - setting to 0 just buys X amount of token at its current price for whatever AVAX specified
        [spend,tokenToBuy],
        walletAddress,
        (int(time.time()) + transactionRevertTime)
        ).buildTransaction({
        'from': walletAddress,
        'value': web3.toWei(float(snipeAVAXAmount), 'ether'), #This is the Token(AVAX) amount you want to Swap from
        'gas': gasAmount,
        ...
        checkTransactionSuccessURL = "https://api.snowtrace.io/api?module=transaction&action=gettxreceiptstatus&txhash=" + txHash + "&apikey=" + snowtraceScanAPIKey
        checkTransactionRequest = requests.get(url = checkTransactionSuccessURL)
        txResult = checkTransactionRequest.json()['status']


        if(txResult == "1"):
            print(style.GREEN + currentTimeStamp + " Successfully bought $" + tokenSymbol + " for " + style.BLUE + str(snipeAVAXAmount) + style.GREEN + " AVAX - TX ID: ", txHash)
...
def foundToken(event):
    try:
        ...
        if(jsonEventContents['args']['token1'] == "0xb31f66aa3c1e785363f0875a1b74e27b85fd66c7"): #check if pair is WAVAX, if not then ignore it
         ...
         #--------------------------------------------MINI AUDIT FEATURE-------------------------------------------------------
            ...
            if(enableMiniAudit == True): #enable mini audit feature: quickly scans token for potential features that make it a scam / honeypot / rugpull etc
                ...
                contractCodeGetRequestURL = "https://api.snowtrace.io/api?module=contract&action=getsourcecode&address=" + tokenAddress + "&apikey=" + snowtraceScanAPIKey
